...
`OpenGL` を有効にする場合：
````
cmake . -DASSIMP_BUILD_TESTS=OFF -DASSIMP_WARNINGS_AS_ERRORS=OFF -DBUILD_SHARED_LIBS=OFF -DCMAKE_BUILD_TYPE=Debug -DAG_USE_OPEN_GL=ON
````

`Metal` を有効にする場合：
````
cmake . -DASSIMP_BUILD_TESTS=OFF -DASSIMP_WARNINGS_AS_ERRORS=OFF -DBUILD_SHARED_LIBS=OFF -DCMAKE_BUILD_TYPE=Debug -DAG_USE_METAL=ON
````

`DirectX` を有効にする場合：
````
cmake . -DASSIMP_BUILD_TESTS=OFF -DASSIMP_WARNINGS_AS_ERRORS=OFF -DBUILD_SHARED_LIBS=OFF -DCMAKE_BUILD_TYPE=Debug -DAG_USE_DIRECT_X=ON -G "Unix Makefiles"
````
